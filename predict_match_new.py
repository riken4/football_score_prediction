...
def load_data(season, data_dir='data'):
    ...
    if not os.path.exists(file):
        raise FileNotFoundError(f"Data file for season {season} not found.")
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(json.dumps({"error": "Usage: python predict_match_new.py <home_team> <away_team> <season>"}))
        sys.exit(1)
    ...
    try:
        ...
        if not stats_home or not stats_away:
            print(json.dumps({"error": "Invalid team names or no data found for selected season."}))
            sys.exit(1)
